...
// Componente de Login
// ==================================
function ColaboracaoLogin({ onLogin }: { onLogin: (user: SupabaseUser) => void }) {
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <div className="relative">
                      ...
                      <Input
                        id="email-input"
                        placeholder="colaborador@velpro.com"
                        {...field}
                        className="pl-9"
                      />
                    </div>
...
function ColaboracaoDashboard({
  ...
  const navItems = [
    { href: "/colaboracao", label: "Leads", icon: Users },
    // Adicionar mais itens aqui no futuro (ex: indicações)
    // { href: "/colaboracao/indicacoes", label: "Indicações", icon: Handshake },
  ];
...
// ==================================
export default function ColaboracaoLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    return <ColaboracaoLogin onLogin={(loggedInUser) => setUser(loggedInUser)} />;
  }

  return (
    <ColaboracaoDashboard user={user} onLogout={() => setUser(null)}>
      {children}
    </ColaboracaoDashboard>
  );
}
