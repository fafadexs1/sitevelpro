...
// Componente de Login
// ==================================
function ColaboradorLogin({ onLogin }: { onLogin: (user: SupabaseUser) => void }) {
...
export default function ColaboradorLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!user) {
    return <ColaboradorLogin onLogin={(loggedInUser) => setUser(loggedInUser)} />;
  }

  return (
    <ColaboradorDashboard user={user} onLogout={() => setUser(null)}>
      {children}
