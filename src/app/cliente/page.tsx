...
interface Referral { id: string; name: string; createdAt: string; status: "pendente" | "aprovado" | "rejeitado" }
interface Contract {
  id: string;
  alias: string;
  address: string;
  ...
  const contracts: Contract[] = [
    {
      ...
      referrals: [
        { id: "R-01", name: "Ana Silva", createdAt: "2025-08-15", status: "aprovado" },
        { id: "R-02", name: "Bruno Costa", createdAt: "2025-09-02", status: "pendente" },
      ]
    },
    {
      id: "C-002",
      ...
      referrals: [
         { id: "R-03", name: "Carla Dias", createdAt: "2025-07-20", status: "rejeitado" }
      ]
    }
  ];
                 ...
                 <div className="mt-8 pt-8 border-t border-border">
                    ...
                    <div className="rounded-xl border border-border">
                        <Table>
                            ...
                            <TableBody>
                                {contract.referrals.length > 0 ? contract.referrals.map(ref => (
                                    <TableRow key={ref.id}>
                                        <TableCell className="font-medium">{ref.name}</TableCell>
                                        <TableCell className="text-sm text-muted-foreground">{new Date(ref.createdAt).toLocaleDateString('pt-BR')}</TableCell>
                                        <TableCell className="text-right">
                                            <Badge className={`${statusColors[ref.status]} capitalize`}>{ref.status}</Badge>
                                        </TableCell>
                                    </TableRow>
                                )) : (
                                    <TableRow>
