...
function AddChannelForm({
  ...
  const form = useForm<ChannelFormData>({
    ...
    defaultValues: { name: "", description: "", logo_file: undefined },
  });
              ...
              <FormItem>
                ...
                <FormControl>
                  <div className="relative flex items-center justify-center w-full">
                    <label htmlFor="dropzone-file-add" className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg bg-accent border-border ${isSubmitting ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-primary'}`}>
                ...
                {field.value && <p className="text-sm text-muted-foreground mt-2">Arquivo selecionado: {field.value.name}</p>}
                <FormMessage />
              </FormItem>
            )}
          />
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <div className="flex items-center gap-4">
                  <Image src={channel.logo_url} alt={channel.name} width={40} height={40} className="rounded-md bg-secondary p-1 object-contain" />
                  <p className="text-xs text-muted-foreground">Logo atual. Envie um novo arquivo abaixo para substituir.</p>
                </div>
                <FormControl>
                  <div className="relative flex items-center justify-center w-full mt-2">
                    <label htmlFor={`dropzone-file-edit-${channel.id}`} className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg bg-accent border-border ${isSubmitting ? 'cursor-not-allowed opacity-50' : 'cursor-pointer hover:border-primary'}`}>
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <Upload className="w-8 h-8 mb-3 text-muted-foreground" />
                        <p className="mb-2 text-sm text-muted-foreground"><span className="font-semibold">Clique para enviar</span> ou arraste</p>
                        <p className="text-xs text-muted-foreground">SVG, PNG, JPG or WEBP (MAX. 5MB)</p>
                ...
                {field.value && <p className="text-sm text-muted-foreground mt-2">Novo arquivo: {field.value.name}</p>}
                <FormMessage />
              </FormItem>
            )}
          />
    ...
    <>
      <header className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Gerencie os canais e marque até 10 como destaque para a página inicial.</p>
        </div>
            ...
            <TableBody>
              {channels.map((channel) => (
                <TableRow key={channel.id}>
                  ...
                  <TableCell className="text-muted-foreground text-xs max-w-sm truncate">{channel.description || 'N/A'}</TableCell>
              ...
              {channels.length === 0 && (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-muted-foreground py-8">Nenhum canal encontrado.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
